...
export async function generarInforme({ tareas, bloqueos, dias = 7, resumen = "", decisiones = [], autor = "" }) {
  ...
  h.tabla({
    ...
    rows: [
      ...
      ["Cerrados en el período", t.cerrados_en_periodo, t.cerrados_internos, t.cerrados_externos,
        t.cerrados_en_periodo ? "Ver punto 2" : "Sin cierres registrados"],
      ["Altas en el período", t.altas_en_periodo, t.altas_internas, t.altas_externas,
  ...
  /* 2. Cerrados */
  sec("Cerrados en el período");
  if (inf.cerrados.length === 0) h.nota("No hay cierres registrados en este período.");
  else h.tabla({
